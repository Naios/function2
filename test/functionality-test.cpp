...
TYPED_TEST(AllSingleMoveAssignConstructTests, AreFreeingResourcesOnDestruction)
{
...
{
  bool result = true;
  typename TestFixture::template left_t<bool(bool)> left([=] (bool in)
  {
    return result && in;
  });
  EXPECT_TRUE(left);
...
TYPED_TEST(AllSingleMoveAssignConstructTests, ProvideItsSignatureAsOperator)
{
  EXPECT_TRUE((
    std::is_same<
      typename TestFixture::template left_t<void()>::return_type,
      void
    >::value));

  EXPECT_TRUE((
    std::is_same<
      typename TestFixture::template left_t<float()>::return_type,
      float
    >::value));

  EXPECT_TRUE((
    std::is_same<
      typename TestFixture::template left_t<void(float, double, int)>
        ::argument_type,
